...
def prox_matrix(v,lambdat,prox_f):
    ...
    v[v != v] = 0
    v[np.isinf(v)] = np.finfo(np.float32).max
    import scipy
    U, S, V = scipy.linalg.svd(v, full_matrices=False)
    S = S.reshape((len(S),1))
...
def objective(X_1, g_2, X_2, g_3, X_3):
    ...
    """
    X_3[X_3 != X_3] = 0
    X_3[np.isinf(X_3)] = np.finfo(np.float32).max
    import scipy
    tmp = scipy.linalg.svd(X_3, compute_uv=0)
...
def rpcaADMM(data):
    ...
    for k in range(1):
        ...
        zold = z
        z = x + tile(-avg(X_1, X_2, X_3) + A*1.0/N, (1, N)) # tile(A,[3,4])，
        U = B
